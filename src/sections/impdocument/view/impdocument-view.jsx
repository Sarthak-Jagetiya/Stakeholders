...
const ImpDocumentForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://10.0.1.38:3000/api/impdocument/${didParam}`);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const apiEndpoint = didParam
        ? `http://10.0.1.38:3000/api/impdocument/${didParam}`
        : 'http://10.0.1.38:3000/api/impdocument/';
