...
export default function TransactionForm() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://10.0.2.128:3000/api/transaction/${idParam}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const apiEndpoint = idParam
        ? `http://10.0.2.128:3000/api/transaction/${idParam}`
        : 'http://10.0.2.128:3000/api/transaction';
