...
export default function RegisterView() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateForm()) {
      ...
      try {
        const response = await axios.post('http://13.126.201.24:3000/api/student/', formData);
  ...
  const handleDocumentSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('http://13.126.201.24:3000/api/document', documentData, {
        headers: {
