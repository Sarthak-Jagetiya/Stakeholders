...
export default function RegisterView() {
  ...
  const fetchCategoryOptions = async () => {
    ...
    try {
      const response = await axios.get('http://10.0.2.54:3000/api/feestructure/unique');
  ...
  const generatePRN = async () => {
    try {
      const response = await axios.get('http://10.0.2.54:3000/api/student/lastStudent/');
      if (response.data.status === 'success') {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://10.0.2.54:3000/api/student/${prnparam}`);
    ...
    if (validateForm()) {
      ...
      try {
        const apiEndpoint = prnparam
          ? `http://10.0.2.54:3000/api/student/${prnparam}`
          : 'http://10.0.2.54:3000/api/student/';
