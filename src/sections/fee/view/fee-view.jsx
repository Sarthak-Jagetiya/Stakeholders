...
export default function FeeView() {
  ...
  const fetchStudentData = useCallback(async () => {
    try {
      const response = await axios.get(`http://15.206.123.134:3000/api/student/${prn}`);
  ...
  const fetchPaidData = useCallback(async () => {
    try {
      const response = await axios.get(`http://15.206.123.134:3000/api/transaction/sum/${prn}`);
      if (response.data.status === 'success') {
  ...
  const fetchUnpaidData = useCallback(async () => {
    try {
      const response = await axios.get(`http://15.206.123.134:3000/api/feestructure/${feeCode}`);
      if (response.data.status === 'success') {
        const { data } = response.data.data;
